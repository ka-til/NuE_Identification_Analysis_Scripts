        ...
        for entry in self.hits:
            if self.geo.omgeo.get(entry.key()).omtype.name == 'mDOM':
                ...
                for hit in entry.data():
                    ...
                    if self.daughter.type == 11 or self.daughter.type == -11:
                        om_prop.append((self.counter, ox, oy, oz, hit.time, hit.charge, d, time_residual, angle, hx, hy, hz))
                    else:
                        om_prop.append((self.counter, ox, oy, oz, hit.time, hit.charge, d, time_residual, angle, hx, hy, hz))
...
def process_frame(frame, geo):
    ...
    om_counter, ox, oy, oz, hit_time, hit_charge, distance, time_residual, angle, hx, hy, hz = extract.get_om_info('true')
    ...
    #reco om level info
    om_counter_reco, ox_reco, oy_reco, oz_reco, hit_time_reco, hit_charge_reco, distance_reco, time_residual_reco, angle_reco, hx_reco, hy_reco, hz_reco = extract.get_om_info('reco')
    ...
    om_reco = {
        "om_counter_reco": om_counter_reco,
        "ox_reco": ox_reco,
        "oy_reco": oy_reco,
        "oz_reco": oz_reco,
        "hit_time_reco": hit_time_reco,
        "hit_charge_reco": hit_charge_reco,
        "distance_reco": distance_reco,
        "time_residual_reco": time_residual_reco,
        "angle_reco": angle_reco,
        "hx_reco": hx_reco,
        "hy_reco": hy_reco,
        "hz_reco": hz_reco,
    }
