...
features = ['RecoEnergy', 'TotalCharge', 'CogDistance', 'Beta1Reco', 'dist_median', 'ang_median']

X = data[features]

train_X, test_X, train_y, test_y = train_test_split(X, y, test_size = 0.2, random_state=0)

model = HistGradientBoostingClassifier(learning_rate=0.0001, max_iter=100000, early_stopping=True, validation_fraction=0.2, n_iter_no_change=10, random_state=0)
...
plt.ylabel('Accuracy')
plt.tight_layout()
plt.savefig('Gradient_Boosting_Classifier_Accuracy_plot_rf.png', dpi=300)
plt.clf()
...
plt.ylabel('Loss')
plt.tight_layout()
plt.savefig('Gradient_Boosting_Classifier_Loss_plot_rf.png', dpi=300)
plt.clf()
...
plt.legend()
plt.tight_layout()
plt.savefig('Gradient_Boosting_Classifier_ROC_curve_rf.png', dpi=300)
plt.clf()
