...
for i in range(20):
    fluctuated_data_nh = [m.fluctuate(method='poisson', random_state=i) for m in fake_data_nh]
    
    best_fit_info_nh_nh = analysis.fit_recursively(
                data_dist=fluctuated_data_nh,
                hypo_maker=template_maker_NO,
                metric=["mod_chi2"],
                external_priors_penalty=None,
                **fit_octant
    ...
    fluctuated_data_ih = [m.fluctuate(method='poisson', random_state=i) for m in fake_data_ih]
    
    best_fit_info_nh_ih = analysis.fit_recursively(
                data_dist=fluctuated_data_ih,
                hypo_maker=template_maker_NO,
                metric=["mod_chi2"],
                external_priors_penalty=None,
                **fit_octant
