...
def beta_n(power, coeff, array):
    '''
    Calculate the beta values(inspired from SNO beta14 variable) for an input array.

    Arguments
    power: degree of he Legendre Polynomial
    coeff: Scaling coefficient
    array: Input array or vector of values

    Returns
    beta: Computed beta(with degree n) value or array
    '''
...
def get_beta_values(ex, ey, ez):
    '''
    Calculate the first five beta values

    Arguments
    ex: x vector component between particle and OM
    ey: y vector component between particle and OM
    ez: z vector component between particle and OM

    Returns
    beta1, beta2, beta3, beta4, beta5: Tuple of the first five computed beta values
    '''
...
def get_cog(charge, x_arr, y_arr, z_arr):
    '''
    Calculate the charge-weighted center of gravity positions

    Arguments
    charge: Array of charges corresponding to each hit
    x_arr: Array of x-coordinates
    y_arr: Array of y-coordinates
    z_arr: Array of z-coordinates

    Returns
    weighted_x, weighted_y, weighted_z: Charge-weighted center of gravity x, y, and z coordinates
    '''
    ...
    #weighted average to get the CoG vertex
...
def upgrade_events(frame, mean_x, mean_y, mean_z):
    '''
    Only select events in the upgrade region which have reconstructed energy <= 100 GeV.

    Arguments
    frame: DAQ or Physics frame containing event data
    mean_x: Mean x-position of the upgrade optical modules
    mean_y: Mean y-position of the upgrade optical modules
    mean_z: Mean z-position of the upgrade optical modules

    Returns
    is_selected: Frame is kept if it meets the upgrade selection criteria
    '''
...
def reco_time(frame, geo):
    '''
    Calculate the time at the reconstructed vertex using the first hit time and reconstructed vertex position(assuming GraphNeT does not return vertex time)
    
    Using the first hit time and the reconstructed vertex to get the time at the reconstructed vertex.
    ...
    Returns
    None: Modifies frame  by adding 'first_hit' and 'reco_vertex_time'
    '''
...
def get_var(frame, geo):
    '''
    Extract variables necessary for NuE identification BDT model and add them to the frame.
    ...
    Returns
    is_selected: True if the event has hits and features were saved to the frame, False otherwise
    '''
...
if __name__ == '__main__':
    ...
    parser.add_argument("--gcd", type=str, default='/home/akatil/GeoCalibDetectorStatus_ICUpgrade.v58.mixed.V1.i3.bz2', help="Give the path to the gcd file")
    ...
    gcd_infile = dataio.I3File(gcd_file)
    ...
    #filelist = sorted(glob(f'{indir}{dataset}/upgrade_genie_level4_queso_*.i3.zst'))
    filelist = sorted(glob(f'{indir}/*.i3.zst'))
