...
def cascade_upgrade_events(frame, mean_x, mean_y, mean_z):
...
def calculate_beta_angle(frame, geo):
    ...
    frame['daughter_type'] = dataclasses.I3Double(daughter.type)
    frame['penergy'] = dataclasses.I3Double(primary[0].energy)
    frame['senergy'] = dataclasses.I3Double(daughter.energy)
...
def create_dataset(infile, outfile, mean_ux, mean_uy, mean_uz, geo):
    ...
    tray.AddModule(cascade_upgrade_events, 'cuts',
                   mean_x = mean_ux,
                   mean_y = mean_uy,
                   mean_z = mean_uz,
